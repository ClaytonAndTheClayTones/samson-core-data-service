...
from models.inventory_intake_job_model import InventoryIntakeJobModel, InventoryIntakeJobStatuses
...
class IngestInventorySnapshotsProcess:
    ...
    def __init__(
        self, 
        posabit_integration: PosabitIntegration,
        manager: Manager,
        process_trace_id: str
    ) -> None:  
        self.posabit_integration = posabit_integration
        self.manager = manager
        self.process_trace_id = process_trace_id
    ...
    def run_process(
        ...
        try:
            ...
            if(isinstance(e, ProcessException)):
                ...
                raise ProcessException(self.process_name, self.process_trace_id, "Arrange", f"Unhandled Exception occured: {e}")
        
        # Step: Arrange 
        try:
            job = self.manager.get_inventory_intake_job_by_id(job_id)
        ...
        try:
            inventory_snapshot_items = self.retrieve_inventory_snapshots(job, integration, self.product_inactivity_threshold_in_days)

        except Exception as e: 
            if(isinstance(e, ProcessException)):
                raise e
            else:
                raise ProcessException(self.process_name, self.process_trace_id, "RetrieveInventorySnapshots", f"Unhandled Exception occured: {e}")
        ...
        try:
            ...
            if(isinstance(e, ProcessException)):
                ...
                raise ProcessException(self.process_name, self.process_trace_id, "InsertInventoryProductSnapshots", f"Unhandled Exception occured: {e}")
    ...
    def retrieve_inventory_snapshots(
        ...
        match(integration.pos_platform):
            case PosPlatforms.Posabit:
                return self.posabit_integration.get_inventory_snapshots(job, integration)
            case _:
