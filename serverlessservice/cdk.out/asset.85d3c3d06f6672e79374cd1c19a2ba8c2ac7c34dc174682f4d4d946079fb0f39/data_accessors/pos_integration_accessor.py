...
class PosIntegrationDataAccessor:
    ...
    def insert(self, model: PosIntegrationCreateModel):
        ...
        db_result: dict[str, Any] = connection.insert('pos_integrations',
                                                      db_model)
    ...
    def select(
        ...
        db_result: SelectQueryResults = connection.select(
            'pos_integrations', search_terms, paging_model)

        results: ItemList[PosIntegrationModel] = ItemList[PosIntegrationModel](
            db_result.paging)
