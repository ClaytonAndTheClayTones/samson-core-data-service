...
def test_build_select_query():
    ...
    like_match_search_term = LikeSearchTerm(
        'test_col_like', 'likesearch', LikeComparatorModes.Like
    )

    in_list_match_search_term = InListSearchTerm(
        'test_col_list', ['abc', 'def', 'ghi'], False
    )

    range_match_search_term = RangeSearchTerm(
        'test_col_range', 111, 222, False
    )
