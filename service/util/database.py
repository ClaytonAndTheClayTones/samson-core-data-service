...
class InListSearchTerm(SearchTerm, Generic[TListSearchable]):
    ...
    def generate_sql(self):
        sqlstring = (
            f'LOWER({self.column_name}) IN (\n'
            if self.ignore_case
            else f'{self.column_name} IN (\n'
        )

        for i, value in enumerate(self.value_list):
            sqlstring += (
                f"\tLOWER('{value}')" if self.ignore_case else f"\t'{value}'"
            )
...
class RangeSearchTerm(SearchTerm, Generic[TRangeSearchable]):
    ...
    def generate_sql(self):
        column_sql: str = (
            f'LOWER({self.column_name})'
            if self.ignore_case
            else f'{self.column_name}'
        )
        value_min_sql: str | None = (
            (
                f"LOWER('{self.value_min}')"
                if self.ignore_case
                else f"'{self.value_min}'"
            )
        ...
        value_max_sql: str | None = (
            (
                f"LOWER('{self.value_max}')"
                if self.ignore_case
                else f"'{self.value_max}'"
            )
