...
class VendorManager():  
    ...
    def search(self, model: VendorSearchModel, paging_model: PagingModel | None = None) -> ItemList[VendorModel]:
        
        result = accessor.select(model, paging_model); 

        return result
    
    def update(self, id: UUID, model: VendorUpdateModel, explicit_null_set: list[str] | None = None):
