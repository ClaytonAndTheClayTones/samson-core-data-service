...
class RetailerLocationManager():  
    def create(self, inboundModel: RetailerLocationCreateModel) -> RetailerLocationModel | None: 
    ...
    def search(self, model: RetailerLocationSearchModel, paging_model: PagingModel | None = None) -> ItemList[RetailerLocationModel]:
        
        result = accessor.select(model, paging_model); 

        return result
    
    def update(self, id: UUID, model: RetailerLocationUpdateModel, explicit_null_set: list[str] | None = None):
