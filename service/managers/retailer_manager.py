...
class RetailerManager():  
    def create(self, inboundModel: RetailerCreateModel) -> RetailerModel | None: 
    ...
    def search(self, model: RetailerSearchModel, paging_model: PagingModel | None = None) -> ItemList[RetailerModel]:
        
        result = accessor.select(model, paging_model); 

        return result
    
    def update(self, id: UUID, model: RetailerUpdateModel, explicit_null_set: list[str] | None = None):
