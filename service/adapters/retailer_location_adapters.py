...
class RetailerLocationDataAdapter:
    ...
    def convert_from_search_model_to_search_terms(
        self, model: RetailerLocationSearchModel
    ) -> list[SearchTerm]:
        ...
        if model.pos_integration_ids is not None:
            search_terms.append(
                InListSearchTerm(
                    'pos_integration_id', model.pos_integration_ids
                )
            )
        ...
        if model.location_state is not None:
            search_terms.append(
                ExactMatchSearchTerm(
                    'location_state', model.location_state, True
                )
            )
        if model.location_city is not None:
            search_terms.append(
                ExactMatchSearchTerm(
                    'location_city', model.location_city, True
                )
            )
        if model.location_country is not None:
            search_terms.append(
                ExactMatchSearchTerm(
                    'location_country', model.location_country, True
                )
            )
    ...
    def convert_from_create_model_to_database_model(
        self, model: RetailerLocationCreateModel
    ) -> dict[str, Any]:
    ...
    def convert_from_update_model_to_database_model(
        self, model: RetailerLocationUpdateModel
    ) -> dict[str, Any]:
        database_model: dict[str, Any] = {
            'name': model.name,
            'pos_integration_id': model.pos_integration_id,
    ...
    def convert_from_model_to_outbound_model(
        self, model: RetailerLocationModel
    ) -> RetailerLocationOutboundModel:
