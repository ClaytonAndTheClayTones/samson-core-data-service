...
from models.vendor_model import VendorCreateModel, VendorDatabaseModel, VendorInboundCreateModel, VendorInboundSearchModel, VendorInboundUpdateModel, VendorModel, VendorOutboundModel, VendorSearchModel, VendorUpdateModel
from util.common import CommonUtilities
from util.database import ExactMatchSearchTerm, InListSearchTerm, LikeComparatorModes, LikeSearchTerm, SearchTerm
 
class VendorDataAdapter():  
    common_utilities: CommonUtilities = CommonUtilities()

    def convert_from_inbound_create_model_to_create_model(self, inbound_create_model: VendorInboundCreateModel) -> VendorCreateModel:
        model = VendorCreateModel(
            ...
            registered_replacement_vendor_id=inbound_create_model.registered_replacement_vendor_id,
    ...
    def convert_from_inbound_update_model_to_create_model(self, inbound_update_model: VendorInboundUpdateModel) -> VendorUpdateModel:
    ...
    def convert_from_inbound_search_model_to_search_model(self, inbound_search_model: VendorInboundSearchModel) -> VendorSearchModel:
        model = VendorSearchModel(
            ids = self.common_utilities.convert_comma_delimited_ids_to_uuid_list(inbound_search_model.ids) if inbound_search_model.ids is not None else None,
            unregistered_vendor_retailer_ids = self.common_utilities.convert_comma_delimited_ids_to_uuid_list(inbound_search_model.unregistered_vendor_retailer_ids) if inbound_search_model.unregistered_vendor_retailer_ids is not None else None,
            registered_replacement_vendor_ids = self.common_utilities.convert_comma_delimited_ids_to_uuid_list(inbound_search_model.registered_replacement_vendor_ids) if inbound_search_model.registered_replacement_vendor_ids is not None else None,
    ...
    def convert_from_search_model_to_search_terms(self, model: VendorSearchModel) -> list[SearchTerm]:
        ...
        if(model.registered_replacement_vendor_ids is not None):
           search_terms.append(InListSearchTerm("registered_replacement_vendor_id", model.registered_replacement_vendor_ids))
     
        if(model.unregistered_vendor_retailer_ids is not None):
           search_terms.append(InListSearchTerm("unregistered_vendor_retailer_id", model.unregistered_vendor_retailer_ids))
    ...
    def convert_from_create_model_to_database_model(self, model: VendorCreateModel) -> dict[str, Any]:
        database_model: dict[str, Any] = {
            "name" : model.name,
            "registered_replacement_vendor_id" : model.registered_replacement_vendor_id,
    ...
    def convert_from_update_model_to_database_model(self, model: VendorUpdateModel) -> dict[str, Any]:
        database_model: dict[str, Any] = {
            "name" : model.name,
            "is_registered" : model.is_registered,
            "hq_state" : model.hq_state,
    ...
    def convert_from_database_model_to_model(self, database_model: dict[str, Any]) -> VendorModel:
        model = VendorModel(
            id = database_model['id'], 
            name = database_model['name'],
            registered_replacement_vendor_id = database_model['registered_replacement_vendor_id'],
    ...
    def convert_from_model_to_outbound_model(self, model: VendorModel) -> VendorOutboundModel:
