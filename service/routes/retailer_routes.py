...
def set_routes(app: FastAPI):
        ...
        @app.post("/retailers", response_model=RetailerOutboundModel, status_code = 201)
        def post_retailer(inbound_create_model: RetailerInboundCreateModel): 
                result = controller.create(inbound_create_model)

                return result

        @app.get("/retailers", response_model= OutboundItemListResponse[RetailerOutboundModel])
        def get_retailers(inbound_search_model: RetailerInboundSearchModel = Depends()) -> OutboundItemListResponse[RetailerOutboundModel]: 
        ...
        def patch_retailer(id: UUID4, inbound_update_model: RetailerInboundUpdateModel): 
