...
from models.retailer_model import RetailerCreateModel, RetailerDatabaseModel, RetailerModel, RetailerSearchModel, RetailerUpdateModel
...
class RetailerDataAccessor():  
    ...
    def select(self, model: RetailerSearchModel, paging_model: PagingModel | None = None) -> ItemList[RetailerModel]:
         
        connection = get_global_configuration().pg_connection
  
        search_terms: list[SearchTerm] = self.adapter.convert_from_search_model_to_search_terms(model)
        db_result: SelectQueryResults = connection.select("retailers", search_terms, paging_model)
    ...
    def update(self, id: UUID, model: RetailerUpdateModel, explicitNullSet: list[str] | None = None):
