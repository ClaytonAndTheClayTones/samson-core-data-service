...
from models.retailer_location_model import RetailerLocationCreateModel, RetailerLocationDatabaseModel, RetailerLocationModel, RetailerLocationSearchModel, RetailerLocationUpdateModel
...
class RetailerLocationDataAccessor():  
    ...
    def insert(self, model: RetailerLocationCreateModel):
        ...
        db_result: dict[str, Any] = connection.insert("retailer_locations", db_model)
    ...
    def select(self, model: RetailerLocationSearchModel, paging_model: PagingModel | None = None) -> ItemList[RetailerLocationModel]:
         
        connection = get_global_configuration().pg_connection
  
        search_terms: list[SearchTerm] = self.adapter.convert_from_search_model_to_search_terms(model)
        db_result: SelectQueryResults = connection.select("retailer_locations", search_terms, paging_model)

        results: ItemList[RetailerLocationModel] = ItemList[RetailerLocationModel](db_result.paging)  
    ...
    def update(self, id: UUID, model: RetailerLocationUpdateModel, explicitNullSet: list[str] | None = None):
