...
from models.common_model import ItemList, OutboundItemListResponse, OutboundResultantPagingModel
...
class RetailerLocationController():  
    def create(self, inbound_model: RetailerLocationInboundCreateModel) -> RetailerLocationOutboundModel | None:
    ...
    def get_by_id(self, id: UUID) -> RetailerLocationOutboundModel | None:
        ...
        if(result is None):
            raise HTTPException(status_code=404, detail = f"RetailerLocation with id {id} not found.")

        response_model: RetailerLocationOutboundModel = adapter.convert_from_model_to_outbound_model(result);  

        return response_model
    
    def search(self, inbound_model: RetailerLocationInboundSearchModel) -> OutboundItemListResponse[RetailerLocationOutboundModel]: 
        ...
        results: ItemList[RetailerLocationModel] = manager.search(search_model, paging_model)
         
        return_result_list = list(map(lambda x: adapter.convert_from_model_to_outbound_model(x), results.items))
    ...
    def update(self, id: UUID, inbound_model: RetailerLocationInboundUpdateModel, explicitNullSet: list[str] | None = None):
              
        explicitNullSet = explicitNullSet or []

        model: RetailerLocationUpdateModel = adapter.convert_from_inbound_update_model_to_create_model(inbound_model)
