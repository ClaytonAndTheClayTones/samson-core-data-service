...
class PosIntegrationController():  
    def create(self, inbound_model: PosIntegrationInboundCreateModel) -> PosIntegrationOutboundModel | None:
    ...
    def get_by_id(self, id: UUID) -> PosIntegrationOutboundModel | None:
        ...
        if(result is None):
            raise HTTPException(status_code=404, detail = f"PosIntegration with id {id} not found.")

        response_model: PosIntegrationOutboundModel = adapter.convert_from_model_to_outbound_model(result);  

        return response_model
    
    def search(self, inbound_model: PosIntegrationInboundSearchModel) -> OutboundItemListResponse[PosIntegrationOutboundModel]: 
        ...
        results: ItemList[PosIntegrationModel] = manager.search(search_model, paging_model)
    ...
    def update(self, id: UUID, inbound_model: PosIntegrationInboundUpdateModel, explicitNullSet: list[str] | None = None):
