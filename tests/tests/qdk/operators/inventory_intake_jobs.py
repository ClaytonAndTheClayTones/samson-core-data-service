...
from tests.qdk.operators.pos_simulator_responses import PosSimulatorResponseCreateModel, PosSimulatorResponseModel, create_pos_simulator_response
...
class InventoryIntakeJobModel():  
    ...
    def __init__(
        ...
        self.simulator_response = PosSimulatorResponseModel(**simulator_response) if simulator_response is not None else None
        self.status_details = status_details
...
def mint_default_inventory_intake_job(
    ...
    if(overrides.simulator_response_id is None and overrides.create_simulator_response_if_null is True):

        new_simulator_response = create_pos_simulator_response(context, overrides.simulator_response, request_operators = request_operators)
...
def update_inventory_intake_job(
    ...
    if(allow_failures == False):
        ...
        assert_object_was_updated(original_object.__dict__, update_model.__dict__, result_dict, ["updated_at", "parent_batch_job", "retailer", "retailer_location", "simulator_response"])
