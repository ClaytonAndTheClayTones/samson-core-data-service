...
def copy_object_when_appropriate(target_object: T, incoming_object: T, explicit_null_sets: list[str] | None = None):
...
def assert_objects_are_equal(actual_object: object, expected_object: object, exempt_properties: list[str] | None = None):
   ...
   for key in actual_object_dict.keys():
      ...
      if(key not in exempt_properties):
         ...
         assert actual_value == expected_value, f"Object Mismatch at key {key}: actual value {actual_value} did not match expected value {expected_value}"
   ...
   for key in final_object.keys():
      ...
      if(key not in exempt_properties):
         if(key in update_object.keys() and update_object[key] is not None):
            ...
            assert actual_value == expected_value, f"Object Mismatch at updated key {key}: final value {actual_value} did not match expected value from update model: {expected_value}" 
         else:
            actual_value = final_object[key]
            expected_value = original_object[key]

            assert actual_value == expected_value, f"Object Mismatch at non-updated key {key}: final value {actual_value} did not match expected value from original model: {expected_value}" 
